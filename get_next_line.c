...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, buffer, 0) < 0)
		return (ft_clear_stash(&stash), NULL);
	while (ft_findnline(stash) == -1)
	{
		...
		if (readc <= 0)
			return (ft_check_last(&stash, &buffer));
		else if (stash)
			ft_update_stash(&stash, &buffer);
		else
			stash = ft_strndup(buffer, 0, ft_strlen(buffer));
		free(buffer);
	}
